...
vec = 1                         #vec~=1 => Scalat light If vec==1 => Vector light  \ro_lambda3 = 0.1;
ro_lambda3 = .1;       
...
for rr in range(0, Rlz):
    ...
    ELy = 1j*EL1/np.sqrt(2)        #Gaussian beam in the e- direction
    ...
    for dd in range(0,steps):
        ...
        #Etot = EL+1j*Gamma_0*1*TSFF(Data.x, Data.y, Data.z, Xobs, Yobs, Zobs, k, sigma)

        #I_tot[dd]= I_tot[dd] + (np.absolute(np.sum(np.sum(np.conjugate(EL)*Etot*dl**2,axis=0)))/(np.pi/2*W0**2))**2/Rlz;


        TSFFV(Xobs, Yobs, Zobs,Data.x, Data.y, Data.z,k0,sigma, Gamma_0,R0)
        Etotx = EL/np.sqrt(2)+1j/2*Gamma_0*TSFFV.Escattx;
        Etoty = 1j*EL/np.sqrt(2)+1j/2*Gamma_0*TSFFV.Escatty;
             
        
        I_tot[dd]= I_tot[dd] + (abs(sum(sum((np.conjugate(EL/np.sqrt(2))*Etotx + np.conjugate(1j*EL/np.sqrt(2))*Etoty)*dl**2)))/(np.pi/2*W0**2))**2/Rlz;
